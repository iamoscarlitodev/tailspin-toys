...
- Document every reusable component's `Props` interface. Add TSDoc for props whose accepted values, accessibility implications, or default behavior are not obvious from their names and types.
...
- Use comments only for intent or non-obvious implementation decisions; do not restate the markup below the comment. Keep comments current with the component behavior.
