...
The repository's contribution and coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the focused [instruction files](.github/instructions/). In particular:

- Comments explain intent, constraints, and non-obvious decisions — not mechanics that are already clear from the code.
...
- TypeScript uses explicit boundary types and the formatting conventions enforced by ESLint.

Keep documentation current with the implementation; stale comments are treated as bugs.
